...
export default class Filterbycategory extends Component {
    ...
    async handleFilterCategory(row, name) {
        let data = { id: row.id, name: name }
    ...
    render() {
        ...
        return (
            <div>
                <Button togglefilter="offcanvas1" className="filter-btn pull-bs-canvas-right" variant="contained" color="primary">Filters </Button>
                                            ...
                                            {!list.SubCategories ? 'loading' : list.SubCategories.map((prop, index) => {
                                                return (
                                                    <div className="card" key={index}>
                                                        ...
                                                        {prop.ChildCategories.map((row, index) => (
                                                            <div key={index} id="collapsefour" className={row.subCatId === this.state.toggle ? "collapse show" : "collapse"}>
                                                                <div className="card-body">
                                                                    <div className="list-group bs-canvas-close" aria-label="Close" onClick={this.handleFilterCategory.bind(this, row, list.name)}>
